...
message_t global_tx_message;

bool true_rx_message_flag = false;
message_t true_rx_message;

bool loop_immutable_rx_message_flag = false;
message_t loop_immutable_rx_message;
...
void loop()
{
	loop_immutable_rx_message_flag = false;
	if (true_rx_message_flag)
	{
		loop_immutable_rx_message_flag = true;
		loop_immutable_rx_message = true_rx_message;

		true_rx_message_flag = false;
	}

	float clock = synchronization();
	phototaxis(clock);
}
...
float synchronization()
{
	...
	if (loop_immutable_rx_message_flag)
	{
		number_of_messages ++;
	...
	global_tx_message.data[1] = ((ticks_until_reset >> 8) & 0x00FF);
	global_tx_message.data[0] = (ticks_until_reset & 0x00FF);
...
int32_t compute_rx_reset_time_adjustment(const uint32_t* last_reset)
{
	...
	uint16_t rx_timer = (((uint16_t) loop_immutable_rx_message.data[1]) << 8)
			| ((uint16_t) (loop_immutable_rx_message.data[0]));
...
{
	global_tx_message.crc = message_crc(&global_tx_message);
	return &global_tx_message;
}
